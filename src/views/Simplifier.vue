...
const parseSuitedHands = (hand, Freq, freqType) => {
  ...
  return suits.map((suit) => {
    ...
    return {
      ...
      [freqType]: parseFloat(Freq),
    };
  });
};

const parseOffSuitHands = (hand, Freq, freqType) => {
  const suits = ["s", "c", "h", "d"];
  const offSuitCombinations = [];
  const isPocketPair = hand.length === 2 && hand[0] === hand[1];
  const baseHand = isPocketPair ? hand.slice(0, 1) : hand.slice(0, 2);
  ...
  for (let i = 0; i < suits.length; i++) {
    ...
    for (let j = 0; j < suits.length; j++) {
      ...
      if (firstSuit !== secondSuit) {
        const offSuitCombination = isPocketPair
          ? `${baseHand}${firstSuit}${baseHand}${secondSuit}`
          : `${baseHand[0]}${firstSuit}${baseHand[1]}${secondSuit}`;
  ...
  // Generate suited combos if characters are different and the hand is of length 2
  if (!isPocketPair && hand.length === 2) {
    console.log("Generating suited combos...");
    const suitedCombos = parseSuitedHands(hand, Freq, freqType);
    console.log("Suited Combos: ", suitedCombos);
    offSuitCombinations.push(...suitedCombos);
  }

  console.log("OffSuit Combinations: ", offSuitCombinations);
  return offSuitCombinations.map((combo) => {
    const hand = typeof combo === "string" ? combo : combo.hand;
    console.log("Ordering hand: ", hand);
    return {
...
watch(raisingRangeRaw, (newVal) => {
  ...
  const handData = newVal.split(",").flatMap((entry) => {
    ...
    // Check if hand is of length 2
    if (handLength === 2) {
      return parseOffSuitHands(hand, raiseFreq, "raiseFreq");
    }

    // Check if hand is of length 3
    if (handLength === 3) {
      // Check if hand includes 's' and not 'o' for suited hands
      if (hand.includes("s") && !hand.includes("o")) {
        return parseSuitedHands(hand, raiseFreq, "raiseFreq");
      }

      // Check if hand includes 'o' for offsuit hands
      if (hand.includes("o")) {
        return parseOffSuitHands(hand, raiseFreq, "raiseFreq");
      }
    }
...
watch(callingRangeRaw, (newVal) => {
  ...
  const handData = newVal.split(",").flatMap((entry) => {
    ...
    if (handLength === 2) {
      return parseOffSuitHands(hand, callFreq, "callFreq");
    }

    if (handLength === 3 && hand.includes("s") && !hand.includes("o")) {
      return parseSuitedHands(hand, callFreq, "callFreq");
    }
